...
def getStoryPointsLeftForActiveSprint (jira_board_id, jira_username, jira_password, jira_url, jira_field_name)
	storyPointsLeft = 0.0
	...
	Net::HTTP.start(sprintUri.host, sprintUri.port,
		...
		issues.each { |issue|
			...
			if (!fields['resolutiondate'] || fields['resolutiondate'].length == 0) && fields[jira_field_name] and fields[jira_field_name].is_a? Numeric
			    storyPointsLeft = storyPointsLeft + fields[jira_field_name].to_f
			end
		}
	end
 
	puts "... story points left: #{storyPointsLeft}"
 
	return storyPointsLeft
end

#getStoryPointsLeftForActiveSprint(jira_board_id, jira_username, jira_password, jira_url, jira_field_name)

SCHEDULER.every '5m', :first_in => 0 do |job|
	count = getStoryPointsLeftForActiveSprint(jira_board_id, jira_username, jira_password, jira_url, jira_field_name)
 
	send_event('jira_story_points_left', current: count)
end